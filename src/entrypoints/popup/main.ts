...
async function setWhatsNewToast() {
  ...
  newReleaseToastLink.addEventListener(
    "click",
    async () => await openTab("whatsNewPage/whatsNew.html")
  );
...
async function setErrorToast() {
  ...
  closeToastButton.addEventListener("click", async () => {
    ...
    await browser.runtime.sendMessage({ action: "closeToast", type: "error" });
  });
}

function setClosePopupButton() {
  const closePopup = document.getElementById(elements.closePopup)!;
  closePopup.setAttribute("title", browser.i18n.getMessage("popupCloseButton"));
...
function setEventListeners() {
  ...
  links.forEach((link) => {
    ...
    element.addEventListener("click", async () => await openTab(link.url));
  });
}

function setFeedbackLink() {
  const feedbackLink = document.getElementById(elements.feedbackLink)!;
  const feedbackText = browser.i18n.getMessage("feedbackTitle") || "Feedback";
  const bagReportText =
